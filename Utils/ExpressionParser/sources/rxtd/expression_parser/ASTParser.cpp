...
IndexType ASTParser::parseBinaryOrPostfix(index minAllowedPrecedence) {
...
IndexType ASTParser::parsePrefixOrTerminalOrGroup() {
	switch (next.type) {
	case Lexer::Type::eEND: {
		throwException(L"Unexpected end of expression");
	}
	case Lexer::Type::eNUMBER: {
		const double number = StringUtils::parseFloat(next.value);
