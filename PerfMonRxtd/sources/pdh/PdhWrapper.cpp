...
namespace rxpm::pdh {
	...
	size_t Snapshot::getItemsCount() const {
		return itemsCount;
	}

	unsigned Snapshot::getCountersCount() const {
	...
	void Snapshot::setBufferSize(size_t size, size_t items) {
	...
	const PDH_RAW_COUNTER& Snapshot::getItem(unsigned counter, size_t index) const {
		return getCounterPointer(counter)[index].RawValue;
	}

	const wchar_t* Snapshot::getName(size_t index) const {
	...
	PdhWrapper::PdhWrapper(rxu::Rainmeter::Logger _log, std::wstring objectName, rxu::OptionParser::OptionList counterTokens) : log(std::move(_log)) {
		QueryWrapper query;
		...
		for (unsigned int inx = 0; inx < counterHandlers.size(); ++inx) {
			counterPath = L"\\" + objectName + L"(*)" + L"\\" + std::wstring { counterTokens.get(inx) };
	...
	size_t PdhWrapper::getCountersCount() const {
		return counterHandlers.size();
	}

	uint32_t PdhWrapper::getItemsCount() const {
		return itemsCount;
	}

	double PdhWrapper::extractFormattedValue(unsigned counter, const PDH_RAW_COUNTER& current, const PDH_RAW_COUNTER& previous) const {
