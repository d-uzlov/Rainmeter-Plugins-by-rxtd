...
namespace kiss_fft {
	...
	public:
		...
		void assign(const std::size_t nfft, const bool inverse) {
			if (nfft != _nfft) {
				...
				for (typename std::vector<complex_type>::iterator it = _twiddles.begin();
				     it != _twiddles.end(); ++it)
					it->imag(-it->imag());
			}
		}
		...
		void transform(
			const complex_type* fft_in, complex_type* fft_out,
			const std::size_t stage = 0,
			const std::size_t fstride = 1, const std::size_t in_stride = 1
		) const {
		...
		void transform_real(const scalar_type* const src, complex_type* const dst) const {
			const std::size_t N = _nfft;
			if (N == 0)
				return;

			// perform complex FFT
			transform(reinterpret_cast<const complex_type*>(src), dst);
	...
	private:
		void kf_bfly2(complex_type* Fout, const size_t fstride, const std::size_t m) const {
			for (std::size_t k = 0; k < m; ++k) {
				const complex_type t = Fout[m + k] * _twiddles[k * fstride];
				Fout[m + k] = Fout[k] - t;
				Fout[k] += t;
			}
		}

		void kf_bfly3(complex_type* Fout, const std::size_t fstride, const std::size_t m) const {
			std::size_t k = m;
			const std::size_t m2 = 2 * m;
			const complex_type *tw1, *tw2;
			complex_type scratch[5];
			const complex_type epi3 = _twiddles[fstride * m];
		...
		void kf_bfly4(complex_type* const Fout, const std::size_t fstride, const std::size_t m) const {
			complex_type scratch[7];
			const scalar_type negative_if_inverse = scalar_type(_inverse ? -1 : +1);
			for (std::size_t k = 0; k < m; ++k) {
				...
				scratch[4] = complex_type(
					scratch[4].imag() * negative_if_inverse,
		...
		void kf_bfly5(complex_type* const Fout, const std::size_t fstride, const std::size_t m) const {
			complex_type *Fout0, *Fout1, *Fout2, *Fout3, *Fout4;
			complex_type scratch[13];
			const complex_type ya = _twiddles[fstride * m];
			const complex_type yb = _twiddles[fstride * 2 * m];
		...
		void kf_bfly_generic(
			complex_type* const Fout,
			const size_t fstride,
			const std::size_t m,
			const std::size_t p
		) const {
			const complex_type* twiddles = &_twiddles[0];
