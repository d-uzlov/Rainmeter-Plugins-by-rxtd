...
namespace kiss_fft {
	...
	inline index calculateNextFastSize(index size, bool real) {
		...
		while (true) {
			...
			while ((m % 5) == 0) m /= 5;
			if (m <= 1)
				break; /* size is completely factorizable by twos, threes, and fives */
			size++;
		}
		if (real) {
			size <<= 1;
	...
	public:
		...
		void transform(const cpx_t* fft_in, cpx_t* fft_out, const std::size_t stage = 0, const std::size_t fstride = 1,
	...
	private:
		...
		mutable std::vector<cpx_t> _tempBuffer;
	};

}
template <typename scalar_t>
kiss_fft::KissFft<scalar_t>::KissFft(const std::size_t nfft, const bool inverse)
	: _nfft(nfft)
	, _inverse(inverse) {
	// fill twiddle factors
	_twiddles.resize(_nfft);
	const scalar_t phinc = (_inverse ? 2 : -2) * std::acos(static_cast<scalar_t>(-1)) / _nfft;
...
template <typename scalar_t>
void kiss_fft::KissFft<scalar_t>::assign(const std::size_t nfft, const bool inverse) {
	if (nfft != _nfft) {
...
template <typename scalar_t>
void kiss_fft::KissFft<scalar_t>::transform(const cpx_t* fft_in, cpx_t* fft_out, const std::size_t stage,
	const std::size_t fstride, const std::size_t in_stride) const {
	...
	switch (p) {
	case 2: kf_bfly2(fft_out, fstride, m);
		break;
	case 3: kf_bfly3(fft_out, fstride, m);
		break;
	case 4: kf_bfly4(fft_out, fstride, m);
		break;
	case 5: kf_bfly5(fft_out, fstride, m);
		break;
	default: kf_bfly_generic(fft_out, fstride, m, p);
		break;
	}
}

template <typename scalar_t>
void kiss_fft::KissFft<scalar_t>::transform_real(const scalar_t* const src, cpx_t* const dst) const {
	...
	const scalar_t pi = std::acos(static_cast<scalar_t>(-1));
	...
	for (std::size_t k = 1; 2 * k < N; ++k) {
		const cpx_t w = static_cast<scalar_t>(0.5) * cpx_t(
			dst[k].real() + dst[N - k].real(),
			dst[k].imag() - dst[N - k].imag());
		const cpx_t z = static_cast<scalar_t>(0.5) * cpx_t(
			dst[k].imag() + dst[N - k].imag(),
			-dst[k].real() + dst[N - k].real());
		const cpx_t twiddle =
...
template <typename scalar_t>
void kiss_fft::KissFft<scalar_t>::kf_bfly2(cpx_t* Fout, const size_t fstride, const std::size_t m) const {
...
template <typename scalar_t>
void kiss_fft::KissFft<scalar_t>::kf_bfly3(cpx_t* Fout, const std::size_t fstride, const std::size_t m) const {
	std::size_t k = m;
...
template <typename scalar_t>
void kiss_fft::KissFft<scalar_t>::kf_bfly5(cpx_t* const Fout, const std::size_t fstride, const std::size_t m) const {
...
template <typename scalar_t>
void kiss_fft::KissFft<scalar_t>::kf_bfly_generic(cpx_t* const Fout, const size_t fstride, const std::size_t m, const std::size_t p) const {
	const cpx_t* twiddles = &_twiddles[0];
	_tempBuffer.resize(p);
	auto &scratchBuf = _tempBuffer;
