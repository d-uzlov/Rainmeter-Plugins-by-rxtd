...
using rxtd::audio_analyzer::CaptureManager;
...
bool CaptureManager::capture() {
	...
	while (true) {
		...
		switch (changes.disconnectionReason) {
			using DR = wasapi_wrappers::implementations::AudioSessionEventsImpl::DisconnectionReason;
		case DR::eNONE:
			break;
...
std::vector<rxtd::winapi_wrappers::GenericComWrapper<IAudioSessionControl>> CaptureManager::getActiveSessions() noexcept(false) {
