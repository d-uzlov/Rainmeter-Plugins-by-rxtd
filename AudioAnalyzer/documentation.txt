...
Improvements in standard measures:
RMS: AudioLevel doesn't really have RMS. It calculates filtered squared wave value instead.
...
Minimalistic examples of usage:
...
Type=Parent
Processing=proc1
Processing_proc1=channels auto | handlers fft, resampler, transformer, mapper, _spec
Handler_fft=type fft | attack 1 | decay 1 | binWidth 20 | overlap 0.75
Handler_resampler=type BandResampler | source fft | freqList flist
Handler_transformer=type BandCascadeTransformer | source resampler
Handler_mapper=type LogarithmicValueMapper |source transformer | offset 0.2 | sensitivity 40
Handler__spec=type spectrogram | length 230 | resolution 8 | source mapper | colors 0.0 0.1, 0.1, 0.1, 0.8 ; 1.0 0.9, 0.85, 0.5, 1.0 ; 1.5 1.0, 0.0, 0.0, 1.5
FreqList_flist=log 75 10 22500
...
Type=Parent
Processing=proc1
Processing_proc1=channels auto | handlers _wave
Handler__wave=type waveform | width 230 |height 75 |resolution 0.1*3*2 | backgroundColor 0.6, 0.6, 0.6, 0.8 | waveColor 0.1, 0.1, 0.1, 2 | lineColor 0.5,0.5,0.5
...
Type=Parent
Processing=proc1
Processing_proc1=channels auto | handlers loudness
Handler_loudness=type loudness | attack 300
...
To make declaration of handlers shorter they are declared in groups named Processing. A Processing declares that for each specified Channel there will be a sequence of specified sound Handlers. If one channel is noted in several Processings then the order of Handlers between different Processings is undefined.

Declarations have following format:
OptionName= suboption1 value1 | suboption2 value2
Same format is used by the Shape meter.
Options are separated by pipe symbol '|', option name is separated from its value by space.
If an suboption value consists of several subvalues, then these subvalues are separated by comma (','): suboption a, b, c.
For example:
Handler_wave=type waveform | backgroundColor 0.6, 0.6, 0.6, 0.8

In every place where value of an option is a number, math operations can be used. Supported operations are +-*/^, parentheses are allowed, all numbers are calculated as floating pointer, division by zero is treated as 0.
For example: 
Handler_wave=type waveform | width (500 + 10)*0.7
Same as:
Handler_wave=type waveform | width 357

Additional info:
Besides numeric values you can get additional info from parent measure using section variable "resolve".
...
"current device"
    ...
    "name" : human readable name of current audio device. For example, "Speakers (Realtek High Definition Audio)".
...
"prop" : get property of a handler. You have to specify Channel, Handler and PropName. See handlers descriptions for details.
Some examples:
...
Source : { Input, Output, <id> }
    ...
    You can specify exact id of some device to capture data from it instead of default devices. You can obtain id with "device list input" and "device list output" section variables, see above for details.

TargetRate : integer >= 0 : 41000
	Target sampling rate. Sampling rate can vary from 41000 (or maybe even less for some devices) to 384000 and potentially more. But you probably don't need that big sampling rate for sound analysis.
...
Processing_<id> : <list of suboptions>
...
Handler_<id> : <list of suboptions>
	Description of sound handler.
	Contains handler type and handler-specific suboptions.
	Possible types are: Loudness, RMS, Peak, FFT, BandResampler, BandCascadeTransformer, WeightedBlur, UniformBlur, FiniteTimeFilter, LogarithmicValueMapper, Spectrogram, Waveform.
	See description of certain types for details.
FreqList_<id> : <list of suboptions>
...
Loudness
Type : { Loudness }
Measures perceived loudness of signal in appliance with EBU R 128 standard.
Options:
attack : float >= 0 : 100
...
resolution : float > 0 : 10
	Time in milliseconds of block in which loudness is found.
Example: type Loudness | resolution 5 | attack 1000
Props:
"block size" : size of the block in which loudness is calculated, in audio points.
...
SizeBy : { BinWidth, Size, SizeExact } : BinWidth
	...
	Do not change. Anything but BinWidth will cause problems when exact settings of a computer where your skin will be running is not known.
...
BinWidth : float > 0 : 100
	...
	Size is automatically set to make actual BinWidth to be equal to target BinWidth.
...
Overlap : float in [0, 1] : 0.5
	Specifies how much FFT windows is moved after each iteration, with 0 meaning that window is moved fully, and 1.0 meaning that window didn't move at all.
...
Example: type fft | attack 10 | resolution 100 | cascadesCount 3
Props:
"size" : size of the FFT.
"attack" : value of attack option
"decay" : value of decay option
...
MinCascade : integer >= 0 : 0
	...
	Set to value in range [1; cascadesCount] to use cascades starting from cascadeMin.
MaxCascade : integer >= 0 : 0
	Max cascade that should be used in band value calculating.
	Set to value in range [cascadeMin; cascadesCount] to use cascades from cascadeMin to cascadeMax.
...
Generates BMP image on disk. Height of the image is determined by number of point in source Handler. Name of the file is: "<folder>\spectrogram-<channel>.bmp". Color of the pixel is linearly interpolated (in sRGB space) between color points, described by either Colors suboption, or by BaseColor and MaxColor if Colors suboption is not present.
Options:
Source : string
	Name of source handler. Should be name of some handler from the end of FFT transform chain (BandResampler, BandCascadeTransformer, LogarithmicValueMapper of some blur or filter, depending on your setup).
...
Colors : (value color) list : <empty>
    A set of points that describe colors of the spectrogram.
    Each point has form (value color). Points are separated by ; (semicolon).
    Example: colors 0.0 0.1, 0.1, 0.1, 0.8 ; 1.0 0.9, 0.85, 0.5, 1.0 ; 1.5 1.0, 0.0, 0.0, 1.5
BaseColor : color : 0,0,0,1
    Only used when Colors suboption is not present.
...
Then only need to resample values into bands and map them onto logarithmic scale.
The chain you need: FFT, BandResampler, LogarithmicValueMapper.
...
The chain you need: FFT, BandResampler, BandCascadeTransformer, LogarithmicValueMapper.
